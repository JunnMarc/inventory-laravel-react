...
import { File, Trash2, Frown } from 'lucide-react';
...
export default function CategoriesIndex() {
  const { props } = usePage<PageProps<{ categories: any; filters: any }>>();
  ...
  const handleAdd = () => router.visit(route('categories.create'));
  ...
  const selectAll = () => {
    if (selectedCategories.length === categories.data.length) {
      ...
      setSelectedCategories(categories.data.map((c: Category) => c.id));
    }
  };

  const handleDelete = () => {
    router.post(route('categories.bulk-destroy'), { ids: selectedCategories }, {
      onSuccess: () => {
        setSelectedCategories([]);
        ...
        <div className="flex flex-col md:flex-row md:space-x-6 space-y-6 md:space-y-0">
          ...
          <section className="flex-1 bg-white dark:bg-neutral-900 rounded shadow-sm overflow-hidden flex flex-col">
            <div className="px-4 pt-4 pb-2">
              <h1 className="text-xl font-semibold text-neutral-800 dark:text-white">Categories</h1>
            </div>
